-- ═══════════════════════════════════════════════════════════════
-- Stories (24-soatlik vaqtinchalik kontent) — SQL Editor uchun
...
create policy "Anyone can view non-expired stories" on public.stories for select
  using (expires_at > now());

drop policy if exists "Users can create their own stories" on public.stories;
create policy "Users can create their own stories" on public.stories for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can delete their own stories" on public.stories;
create policy "Users can delete their own stories" on public.stories for delete
...
create policy "Viewers can see their own view records" on public.story_views for select
  using (auth.uid() = viewer_id);

drop policy if exists "Story owners can see who viewed their stories" on public.story_views;
create policy "Story owners can see who viewed their stories" on public.story_views for select
...
create policy "Users can record their own views" on public.story_views for insert
