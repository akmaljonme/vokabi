import { ArrowRight, BookOpen, Headphones, Award, Users, CheckCircle2, Star, ChevronDown, Zap, Globe, TrendingUp, Shield, Sparkles, Crown, Brain, BarChart3, Target, Mic, PenTool, MessageSquarePlus, Rocket, Trophy, GraduationCap, Gamepad2, Video, Bot, Languages, Flame, Medal, Heart } from 'lucide-react';
...
/* ─── Marquee ticker ─── */
const MarqueeTicker = ({ items, speed = 30 }: { items: string[]; speed?: number }) => {
  ...
  return (
    <div className="overflow-hidden whitespace-nowrap">
      ...
      >
        {doubled.map((item, i) => (
          <span key={i} className="text-lg font-display font-bold text-muted-foreground/15 tracking-tight">{item}</span>
        ))}
      </motion.div>
    </div>
  );
...
export const LandingPage = ({ onStartTest, onGoToVocabulary }: LandingPageProps) => {
  ...
  const fireConfetti = useCallback(() => {
    confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 }, colors: ['#e11d48', '#8b5cf6', '#06b6d4', '#f59e0b', '#10b981'] });
  }, []);
  ...
  const features = [
    ...
    { icon: Mic, title: 'Speaking Practice', desc: "Ovozingizni yozib oling, AI real-time tahlil qiladi.", color: '270 76% 55%', large: true, pro: true },
    { icon: Zap, title: 'Grammatika', desc: "Fe'l zamonlari va gap tuzilishi testlari", color: '358 84% 50%' },
  ];

  const competitorComparison = [
    { feature: "AI Test Generatsiya", vokabi: true, scoreup: false, selflearn: false, duolingo: false },
    { feature: "21+ O'yin", vokabi: true, scoreup: false, selflearn: false, duolingo: true },
    { feature: "Writing AI Baholash", vokabi: true, scoreup: false, selflearn: false, duolingo: false },
    { feature: "Speaking AI Tahlil", vokabi: true, scoreup: false, selflearn: false, duolingo: true },
    { feature: "CEFR A1-C1 Testlar", vokabi: true, scoreup: true, selflearn: true, duolingo: false },
    { feature: "Real-time Leaderboard", vokabi: true, scoreup: false, selflearn: false, duolingo: true },
    { feature: "Telegram Bot Support", vokabi: true, scoreup: false, selflearn: false, duolingo: false },
    { feature: "O'zbekcha Interfeys", vokabi: true, scoreup: true, selflearn: true, duolingo: false },
    { feature: "Daily Challenges", vokabi: true, scoreup: false, selflearn: false, duolingo: true },
    { feature: "Bepul Rejim", vokabi: true, scoreup: true, selflearn: true, duolingo: true },
  ];

  const gameShowcase = [
    '🔗 Word Match', '🐝 Spelling Bee', '⚔️ Grammar Battle', '🃏 Flashcards', '💀 Hangman',
    '📝 Sentence Builder', '🎧 Listening Quiz', '🎭 Idiom Master', '🔤 Last Word (AI)',
    '🧩 Crossword', '🔀 Word Scramble', '✏️ Fill in the Blank', '🔄 Synonyms',
    '📍 Prepositions', '⏰ Verb Tenses', '🔥 Phrasal Verbs', '🧲 Collocations',
    '👅 Tongue Twisters', '⚡ Reading Speed', '🧠 Memory Cards', '✅ True or False',
  ];
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      <section ref={heroRef} className="relative min-h-[100vh] flex items-center overflow-hidden">
        ...
        <motion.div style={{ y: heroY, opacity: heroOpacity, scale: heroScale }} className="container mx-auto px-4 relative z-10">
          <div className="max-w-5xl mx-auto text-center">
            ...
            >
              21+ o'yin, AI-powered testlar, Writing & Speaking baholash — barchasi bir platformada.
              IELTS va CEFR imtihonlariga tayyorlaning.
            </motion.p>
            ...
            >
              {['Kredit karta shart emas', '21+ o\'yin', 'AI bilan tahlil', 'Tezkor natijalar'].map((text) => (
        ...
        <motion.a href="#stats" className="absolute bottom-8 left-1/2 -translate-x-1/2" animate={{ y: [0, 8, 0] }} transition={{ duration: 2, repeat: Infinity }}>
      ...
      <section className="py-10 border-b border-border/30 overflow-hidden">
        ...
        <MarqueeTicker items={['IELTS', 'TOEFL', 'Cambridge', 'Goethe', 'DELF', 'CEFR', 'British Council', 'ETS', 'Pearson', 'Trinity']} speed={25} />
      </section>
      ...
      <section className="py-28 lg:py-36 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <FadeUp className="text-center mb-16">
            <span className="premium-badge mb-4 inline-flex"><Gamepad2 className="w-3.5 h-3.5" /> 21+ O'YINLAR</span>
          ...
          <FadeUp className="text-center">
            ...
            >
              <Gamepad2 className="w-4 h-4 mr-2 inline" /> Barcha o'yinlarni ko'rish
            </motion.button>
          </FadeUp>
        </div>
      </section>
      ...
      <section className="py-28 lg:py-36 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <FadeUp className="text-center mb-16">
            <span className="premium-badge mb-4 inline-flex"><Trophy className="w-3.5 h-3.5" /> TAQQOSLASH</span>
          ...
          <FadeUp>
            <div className="max-w-4xl mx-auto overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    <th className="text-left p-3 text-sm font-medium text-muted-foreground">Xususiyat</th>
                    ...
                    <th className="p-3 text-center text-sm font-medium text-muted-foreground">ScoreUp</th>
                    <th className="p-3 text-center text-sm font-medium text-muted-foreground">Self-Learn</th>
                    <th className="p-3 text-center text-sm font-medium text-muted-foreground">Duolingo</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    >
                      ...
                      <td className="p-3 text-center">
                        <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-primary/10">
                          {row.vokabi ? <CheckCircle2 className="w-4 h-4 text-primary" /> : <span className="text-muted-foreground">—</span>}
                        </span>
                      </td>
                      <td className="p-3 text-center">
                ...
                <FadeUp key={level.level} delay={index * 0.08}>
                  <Card3D className="level-card text-center group w-full" glowColor={c}>
                    <motion.div whileTap={{ scale: 0.96 }} onClick={handleStartTest} className="cursor-pointer">
                      <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl border text-xl font-display font-bold mb-3 group-hover:scale-110 transition-transform duration-300"
                        style={{ background: `hsl(${c} / 0.1)`, color: `hsl(${c})`, borderColor: `hsl(${c} / 0.2)` }}>
                        {level.level}
                      </div>
            ...
            {[
              ...
              <FadeUp key={index} delay={index * 0.15}>
                <Card3D className="relative text-center group p-8 rounded-2xl border border-border/50 bg-card" glowColor={item.color}>
                  {index < 2 && <div className="hidden md:block absolute top-1/2 -right-4 w-8 h-0.5 bg-border/50 z-10" />}
                  <motion.div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-6" style={{ background: `hsl(${item.color} / 0.1)` }}
                    whileHover={{ rotate: [0, -5, 5, 0], transition: { duration: 0.5 } }}>
                    <item.icon className="w-7 h-7" style={{ color: `hsl(${item.color})` }} />
                  </motion.div>
                  <div className="text-[11px] font-bold mb-2 tracking-[0.2em]" style={{ color: `hsl(${item.color})` }}>STEP {item.step}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            <FadeUp delay={0}>
              <Card3D className="card-elevated p-8 h-full" glowColor="215 16% 47%">
                ...
                <ul className="space-y-3 mb-8">
                  {['Kuniga 3 ta test', 'Reading & Listening testlar', "Lug'at & Grammatika testlar", '21+ o\'yin', 'Natijalar tarixi'].map((f) => (
                    <li key={f} className="flex items-center gap-3 text-sm"><CheckCircle2 className="w-4 h-4 text-primary flex-shrink-0" />{f}</li>
                  ))}
                </ul>
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleStartTest} className="w-full btn-outline py-3 text-sm">
                  {user ? 'Testni Boshlash' : 'Bepul Boshlash'}
                </motion.button>
              </Card3D>
            </FadeUp>
            ...
            <FadeUp delay={0.1}>
              <Card3D className="relative card-elevated p-8 h-full border-primary/30 bg-primary/[0.02]" glowColor="358 84% 50%">
                ...
                <ul className="space-y-3 mb-8">
                  {['Cheksiz testlar', 'AI tahlil & feedback', 'Video tavsiyalar', 'Writing AI baholash (9-ball)', 'Speaking AI tahlil', 'Sertifikat yuklab olish', 'Priority support'].map((f) => (
                    <li key={f} className="flex items-center gap-3 text-sm"><CheckCircle2 className="w-4 h-4 text-primary flex-shrink-0" />{f}</li>
                  ))}
                </ul>
                <motion.a href="https://t.me/vokabi_bot" target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            ...
            {(feedbacks.length > 0 ? feedbacks : [
              ...
              return (
                <FadeUp key={fb.id || index} delay={index * 0.08}>
                  <motion.div whileHover={{ y: -4 }} className="rounded-2xl p-7 bg-white/[0.03] backdrop-blur-sm border border-white/[0.06] h-full hover:bg-white/[0.05] transition-colors">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center font-semibold text-sm text-primary">{initials}</div>
                      <div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-2xl mx-auto space-y-3">
            {[
              ...
              { q: 'Qanday turdagi testlar mavjud?', a: "6 xil test turi mavjud: Lug'at, Grammatika, Reading, Listening, Writing va Speaking." },
              { q: 'Nechta o\'yin bor?', a: "21+ o'yin mavjud: Word Match, Spelling Bee, Grammar Battle, Crossword, Memory Cards, Tongue Twisters va boshqalar." },
              { q: 'Pro versiya nimalar beradi?', a: "Cheksiz testlar, AI tahlil, writing/speaking AI baholash, sertifikat va batafsil statistika." },
              { q: "Pro qanday olsa bo'ladi?", a: 'Telegram botimizga yozing: @vokabi_bot' },
              { q: 'AI tahlil qanday ishlaydi?', a: "Sun'iy intellekt natijalaringizni tahlil qilib, zaif tomonlaringizni aniqlaydi va shaxsiy reja tuzadi." },
            ].map((faq, index) => (
          ...
          <FadeUp>
            <div className="max-w-3xl mx-auto flex flex-col md:flex-row items-center gap-8 p-8 rounded-3xl border border-primary/20 bg-primary/[0.02]">
              ...
              <div className="flex-1 text-center md:text-left">
                <h3 className="text-xl font-display font-bold mb-2">Telegram kanalimizga qo'shiling!</h3>
                <p className="text-muted-foreground text-sm">Yangiliklar, maslahatlar va maxsus takliflar — @vokabi</p>
              </div>
              <div className="flex gap-3">
                <motion.a whileHover={{ scale: 1.05 }} href="https://t.me/vokabi" target="_blank" rel="noopener noreferrer"
                  className="btn-primary px-6 py-2.5 text-sm">📢 Kanal</motion.a>
                <motion.a whileHover={{ scale: 1.05 }} href="https://t.me/vokabi_bot" target="_blank" rel="noopener noreferrer"
                  className="btn-outline px-6 py-2.5 text-sm">🤖 Bot</motion.a>
              </div>
            </div>
          </FadeUp>
        </div>
      ...
      <section className="py-28 lg:py-36 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <FadeUp>
            <motion.div initial={{ scale: 0.9 }} whileInView={{ scale: 1 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
