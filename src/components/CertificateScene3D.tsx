...
export const CertificateScene3D = () => {
  ...
  useEffect(() => {
    ...
    blueLight.position.set(-5, 3, 5); scene.add(blueLight);
    const goldLight = new THREE.PointLight(0xf7c948, 2.5, 25);
    goldLight.position.set(5, -3, 5); scene.add(goldLight);
    const greenLight = new THREE.PointLight(0x48d9a4, 2, 20);
    greenLight.position.set(0, -5, 3); scene.add(greenLight);
    ...
    const loadTex = (src: string) => {
      ...
      img.onload = () => {
        ...
        const scale = Math.min(512 / img.width, 724 / img.height);
        const dw = img.width * scale;
        const dh = img.height * scale;
        ctx.drawImage(img, (512 - dw) / 2, (724 - dh) / 2, dw, dh);
    ...
    // IELTS cert (portrait — like a real A4 cert)
    const certW = 3.0, certH = 4.2, certD = 0.08;

    const ieltsTex = loadTex(IELTS_IMG);
    ...
    const ieltsMats = [
      ...
      new THREE.MeshStandardMaterial({ color: 0xf0f0f0, roughness: 0.3, metalness: 0.1 }),
      new THREE.MeshStandardMaterial({ color: 0xf0f0f0, roughness: 0.3, metalness: 0.1 }),
      new THREE.MeshStandardMaterial({ map: ieltsTex, roughness: 0.08, metalness: 0.05 }),
    ...
    ieltsMesh.position.set(-1.1, 0.5, 0.2);
    ieltsMesh.rotation.set(0.1, -0.25, 0.06);
    ieltsMesh.castShadow = true;
    scene.add(ieltsMesh);

    // O'zbek sertifikat (portrait)
    const certTex = loadTex(CERT_IMG);
    const certGeo = new THREE.BoxGeometry(certW, certH, certD);
    const certMats = [
      new THREE.MeshStandardMaterial({ color: 0xf0f0f0, roughness: 0.3, metalness: 0.1 }),
      new THREE.MeshStandardMaterial({ color: 0xf0f0f0, roughness: 0.3, metalness: 0.1 }),
      new THREE.MeshStandardMaterial({ color: 0xf0f0f0, roughness: 0.3, metalness: 0.1 }),
      new THREE.MeshStandardMaterial({ color: 0xf0f0f0, roughness: 0.3, metalness: 0.1 }),
      new THREE.MeshStandardMaterial({ map: certTex, roughness: 0.08, metalness: 0.05 }),
      new THREE.MeshStandardMaterial({ color: 0xe0e0e0, roughness: 0.4, metalness: 0.1 }),
    ];
    const certMesh = new THREE.Mesh(certGeo, certMats);
    certMesh.position.set(1.1, -0.5, -0.2);
    certMesh.rotation.set(-0.08, 0.3, -0.05);
    ...
    for (let i = 0; i < count; i++) {
      ...
      const t = Math.random();
      if (t < 0.33) { colors[i*3]=0.97; colors[i*3+1]=0.79; colors[i*3+2]=0.28; }
      else if (t < 0.66) { colors[i*3]=0.28; colors[i*3+1]=0.85; colors[i*3+2]=0.64; }
      else { colors[i*3]=0.31; colors[i*3+1]=0.56; colors[i*3+2]=0.97; }
    }
    particleGeo.setAttribute("position", new THREE.BufferAttribute(positions, 3));
    particleGeo.setAttribute("color", new THREE.BufferAttribute(colors, 3));
    const particleMat = new THREE.PointsMaterial({ size: 0.05, vertexColors: true, transparent: true, opacity: 0.7 });
    ...
    let autoRotate = true, manualRotY = 0;
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (isDragging) {
        manualRotY += (e.clientX - lastMouseX) * 0.008;
    ...
    const onMouseUp = () => {
      isDragging = false; mount.style.cursor = "grab";
    ...
    let lastTouchX = 0;
    mount.addEventListener("touchstart", (e) => { lastTouchX = e.touches[0].clientX; autoRotate = false; });
    mount.addEventListener("touchmove", (e) => {
      manualRotY += (e.touches[0].clientX - lastTouchX) * 0.01;
    ...
    const animate = () => {
      ...
      targetRX += (mouseY * 0.2 - targetRX) * 0.04;
      targetRY += (mouseX * 0.2 - targetRY) * 0.04;

      const baseY = autoRotate ? autoAngle : manualRotY;

      ieltsMesh.rotation.x = 0.1 + Math.sin(t * 0.6) * 0.05 + targetRX * 0.4;
      ieltsMesh.rotation.y = baseY - 0.25 + Math.sin(t * 0.4) * 0.04 + targetRY * 0.4;
      ieltsMesh.position.y = 0.5 + Math.sin(t * 0.9) * 0.12;

      certMesh.rotation.x = -0.08 + Math.sin(t * 0.5 + 1.5) * 0.05 + targetRX * 0.3;
      certMesh.rotation.y = baseY + 0.3 + Math.sin(t * 0.35 + 0.8) * 0.04 + targetRY * 0.3;
      certMesh.position.y = -0.5 + Math.sin(t * 0.8 + 2) * 0.10;

      particles.rotation.y = t * 0.04;
      particles.rotation.x = t * 0.02;

      blueLight.position.x = Math.sin(t * 0.5) * 5;
      goldLight.position.x = Math.cos(t * 0.4) * 5;
  ...
  return (
    <div
      ...
      style={{ cursor: "grab", minHeight: "460px" }}
    />
  );
};
