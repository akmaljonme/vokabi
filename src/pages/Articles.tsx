...
import { BookOpen, Clock, Search, CheckCircle } from "lucide-react";
...
const TOPICS = ["Barchasi", "Daily Life", "Food", "Health", "Technology", "Environment", "Science", "Culture", "Education", "Sports", "Travel", "Society", "Psychology", "Finance", "Nature", "Animals", "Family", "Hobbies", "Politics", "Economics", "Philosophy", "Literature"];
...
export default function Articles() {
  ...
  useEffect(() => {
    if (user) {
      supabase.from("article_progress").select("*").eq("user_id", user.id)
        .then(({ data }) => {
          const map: Record<string, any> = {};
          (data || []).forEach((p: any) => { map[p.article_id] = p; });
          setProgress(map);
        });
    }
  }, [user]);
  ...
  const readCount = Object.values(progress).filter((p: any) => p.is_read).length;
  const testedCount = Object.values(progress).filter((p: any) => p.test_score !== null).length;
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        {user && (
          <div className="flex gap-4 mb-6 text-sm text-muted-foreground">
            ...
            <span>✅ O'qilgan: <b>{readCount}</b></span>
            <span>🎯 Test yechilgan: <b>{testedCount}</b></span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((article, i) => {
            ...
            const hasTested = prog?.test_score !== null && prog?.test_score !== undefined;
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  {hasTested ? (
                    <span className="text-xs font-semibold text-green-500">
                      ✅ {prog.test_score}/{prog.test_total}
                    </span>
                  ) : isRead ? (
                ...
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1"><BookOpen className="w-3 h-3" /> {article.wordCount} so'z</span>
                  <span className="ml-auto text-[10px] px-2 py-0.5 rounded-full bg-muted">{article.topic}</span>
                </div>
              </motion.div>
            );
          })}
