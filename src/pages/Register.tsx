...
// step 0-3 = onboarding, step 4 = register form
...
const Register = () => {
  ...
  // Can go next?
  const canNext = () => {
    if (step === 0) return !!prefs.target_language;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Preferences ni localStorage ga saqla (DB ustunlari bo'lmasa ham ishlaydi)
      localStorage.setItem("vokabi_prefs", JSON.stringify({
        ...prefs,
        onboarding_done: true,
      }));

      // Profile ni yangilashga urinib ko'r
      try {
        const { data: { user: newUser } } = await supabase.auth.getUser();
        if (newUser) {
          await supabase.from("profiles").update({
      ...
      } catch { /* localStorage fallback ishlaydi */ }

      navigate("/login", {
        state: { message: "Hisob yaratildi! Emailingizni tasdiqlang va kirting." },
      });
    } catch {
      setError("Kutilmagan xatolik yuz berdi.");
    } finally {
  ...
  const selectedLang = LANGUAGES.find(l => l.id === prefs.target_language);

  // ── Onboarding steps 0–3 ──────────────────────────────────
  if (step < 4) {
    return (
      <div className="fixed inset-0 bg-background flex flex-col items-center justify-start p-4 overflow-auto">
        <div className="w-full max-w-lg flex flex-col min-h-screen py-6">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={step}
              initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.18 }}
              className="bg-card border border-border rounded-2xl p-5 flex-1 overflow-y-auto mb-4">
              ...
              {step === 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {LANGUAGES.map(lang => (
                      <button key={lang.id}
                        onClick={() => setPrefs(p => ({ ...p, target_language: lang.id }))}
                        className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all text-left
                          ${prefs.target_language === lang.id
                            ? "border-primary bg-primary/5"
                            : "border-border hover:border-primary/40 hover:bg-muted/50"}`}>
                        ...
                        {prefs.target_language === lang.id && <Check className="w-4 h-4 text-primary ml-auto shrink-0" />}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {step === 1 && (
                <div>
                  ...
                  <p className="text-muted-foreground text-sm mb-4">Sizga mos kontent ko'rsatishga yordam beradi</p>
                  <div className="grid grid-cols-2 gap-2">
                    {PURPOSES.map(p => (
                      <button key={p.id}
                        onClick={() => setPrefs(pr => ({ ...pr, learning_purpose: p.id }))}
  ...
  return (
    <AuthLayout
      ...
      subtitle={`${LANGUAGES.find(l => l.id === prefs.target_language)?.flag} ${LANGUAGES.find(l => l.id === prefs.target_language)?.name} o'rganuvchi`}
    >
      {/* Selected prefs summary */}
      <div className="flex flex-wrap gap-2 mb-5">
        {[
          { label: LANGUAGES.find(l => l.id === prefs.target_language)?.name, emoji: LANGUAGES.find(l => l.id === prefs.target_language)?.flag },
          { label: PURPOSES.find(p => p.id === prefs.learning_purpose)?.label, emoji: PURPOSES.find(p => p.id === prefs.learning_purpose)?.emoji },
          { label: prefs.current_level, emoji: "📊" },
          { label: GOALS.find(g => g.id === prefs.learning_goal)?.label, emoji: GOALS.find(g => g.id === prefs.learning_goal)?.emoji },
        ].map((item, i) => item.label && (
