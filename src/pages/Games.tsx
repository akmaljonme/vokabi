...
type GameType = 'menu' | 'wordmatch' | 'spelling' | 'grammar' | 'flashcards' | 'hangman' | 'sentence' | 'listening' | 'idiom' | 'lastword' | 'crossword' | 'scramble' | 'fillblank' | 'synonym' | 'preposition' | 'verbtense' | 'phrasal' | 'collocations' | 'tonguetwister' | 'readingspeed' | 'memory' | 'truefalse' | 'tournament' | 'friends' | 'stats';
...
const games = [
  ...
  { id: 'grammar' as const, title: 'Grammar Battle', desc: 'Grammatika bo\'yicha savol-javob', icon: '⚔️', color: 'from-purple-500 to-pink-500' },
  ...
  { id: 'lastword' as const, title: 'Last Word', desc: 'Oxirgi harf bilan so\'z toping — AI!', icon: '🔤', color: 'from-orange-500 to-red-500', badge: 'AI' },
  { id: 'crossword' as const, title: 'Crossword', desc: 'Krossword jumboqini yeching', icon: '🧩', color: 'from-sky-500 to-blue-500', badge: 'NEW' },
  { id: 'scramble' as const, title: 'Word Scramble', desc: 'Aralash harflardan so\'z toping', icon: '🔀', color: 'from-violet-500 to-purple-500', badge: 'AI' },
  { id: 'fillblank' as const, title: 'Fill in the Blank', desc: 'Bo\'sh joyni to\'ldiring', icon: '✏️', color: 'from-teal-500 to-cyan-500', badge: 'AI' },
  { id: 'synonym' as const, title: 'Synonyms', desc: 'Sinonimlarni toping', icon: '🔄', color: 'from-lime-500 to-green-500' },
  { id: 'preposition' as const, title: 'Preposition Master', desc: 'Predloglarni to\'g\'ri ishlating', icon: '📍', color: 'from-pink-500 to-rose-500' },
  { id: 'verbtense' as const, title: 'Verb Tenses', desc: 'Fe\'l zamonlarini o\'rganing', icon: '⏰', color: 'from-amber-500 to-yellow-500', badge: 'AI' },
  { id: 'phrasal' as const, title: 'Phrasal Verbs', desc: 'Phrasal verblarni o\'rganing', icon: '🔥', color: 'from-red-500 to-orange-500' },
  { id: 'collocations' as const, title: 'Collocations', desc: 'So\'z birikmalarini o\'rganing', icon: '🧲', color: 'from-cyan-500 to-blue-500' },
  { id: 'tonguetwister' as const, title: 'Tongue Twisters', desc: 'Til o\'gishtiruvchilarni mashq qiling', icon: '👅', color: 'from-purple-500 to-indigo-500' },
  { id: 'readingspeed' as const, title: 'Reading Speed', desc: 'O\'qish tezligingizni oshiring', icon: '⚡', color: 'from-green-500 to-lime-500' },
  { id: 'memory' as const, title: 'Memory Cards', desc: 'Juft kartalarni toping', icon: '🧠', color: 'from-blue-500 to-indigo-500' },
  { id: 'truefalse' as const, title: 'True or False', desc: 'To\'g\'ri yoki noto\'g\'ri?', icon: '✅', color: 'from-emerald-500 to-green-500' },
];
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 border-b border-border/50 bg-background/80 backdrop-blur-xl">
        <div className="container mx-auto px-4 h-16 flex items-center gap-4">
          ...
          <span className="ml-auto text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-bold">{games.length} ta o'yin</span>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8">
        <AnimatePresence mode="wait">
          {activeGame === 'menu' && (
            <motion.div key="menu" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-5xl mx-auto mb-8">
                <button onClick={() => setActiveGame('tournament')} className="group p-4 rounded-2xl border border-amber-500/30 bg-amber-500/5 hover:bg-amber-500/10 transition-all text-center">
                ...
                <button onClick={() => setActiveGame('friends')} className="group p-4 rounded-2xl border border-primary/30 bg-primary/5 hover:bg-primary/10 transition-all text-center">
                ...
                <button onClick={() => setActiveGame('stats')} className="group p-4 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 hover:bg-emerald-500/10 transition-all text-center">
                ...
                <button onClick={() => navigate('/leaderboard')} className="group p-4 rounded-2xl border border-fuchsia-500/30 bg-fuchsia-500/5 hover:bg-fuchsia-500/10 transition-all text-center">
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      {'badge' in game && game.badge && (
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${game.badge === 'AI' ? 'bg-primary text-primary-foreground' : 'bg-emerald-500 text-white'}`}>{game.badge}</span>
                      )}
                    </div>
          ...
          {activeGame === 'crossword' && <CrosswordGame key="cw" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'scramble' && <WordScrambleGame key="ws" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'fillblank' && <FillBlankGame key="fb" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'synonym' && <SynonymGame key="sy" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'preposition' && <PrepositionGame key="pp" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'verbtense' && <VerbTenseGame key="vt" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'phrasal' && <PhrasalVerbGame key="pv" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'collocations' && <CollocationsGame key="cl" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'tonguetwister' && <TongueTwisterGame key="tt" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'readingspeed' && <ReadingSpeedGame key="rs" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'memory' && <MemoryCardGame key="mc" onBack={() => setActiveGame('menu')} />}
          {activeGame === 'truefalse' && <TrueFalseGame key="tf" onBack={() => setActiveGame('menu')} />}
